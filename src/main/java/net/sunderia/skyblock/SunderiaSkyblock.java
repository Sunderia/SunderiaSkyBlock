...
public class SunderiaSkyblock extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        registerListeners(new Reflections("net.sunderia.skyblock").getSubTypesOf(Listener.class).stream().map(clazz -> (Class<? extends Listener>) clazz).collect(Collectors.toSet()));
        registerCommands(new Reflections("net.sunderia.skyblock").getTypesAnnotatedWith(CommandInfo.class).stream().map(clazz -> (Class<? extends CommandExecutor>) clazz).collect(Collectors.toSet()));
        Events.update();
